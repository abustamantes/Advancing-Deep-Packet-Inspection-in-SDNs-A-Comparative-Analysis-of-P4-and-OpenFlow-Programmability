...
def setUp(grpc_addr='192.168.30.149:9559',
        ...
        for pktin in pktin_lst:
            ...
            if packet.haslayer(IP):
                ...
                if packet.haslayer(TCP):
                    ...
                    if tcp_segment.dport == 3306:
                        blacklist_commands = read_blacklist("/home/anthony/SQL_TEST/p4runtime_based_implementation/p4-guide/flowcache/sql_blacklist.txt")
                        if process_sql(packet, blacklist_commands):
                        ...
                        logger.info("TCP packet not on port 3306, hence, we assumed that the traffic is not SQL.")
                else:
                    logger.info("Not a SQL packet, moving on.")
            else:
